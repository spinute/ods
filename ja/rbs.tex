...
上の例から、直感的には、$0,\ldots,14$をランダムに並び替え、その要素を順に二分探索木に入れていくと、（\figref{rbs-lvc}の右のもののような）とてもバランスの良い木ができることが多く、（\figref{rbs-lvc}の左のもののような）非常にバランスの悪い木ができることはあまりないと分かる。
...
ここで、\emph{ランダムな置換}
\ejindex{permutation!random}{ちかん@置換!ランダム}%
...
ふたつめは、木に含まれない要素を探す場合の探索経路の長さの期待値について主張する。 % YJ removed ambiguousity
