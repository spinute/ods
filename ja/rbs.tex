...
#Treap#のノードは値#x#を持つ点で#BinarySearchTree#に似ているが、それに加えて値#x#に対して一意な\emph{優先度 (priority)}#p#を持つ。
この#p#はランダムに割当てられる。
...
回転には\emph{右回転 (right rotation)}と\emph{左回転 (left rotation)}の二種類があって、それぞれ#w#が#u#の右の子であるか、左の子であるかに対応する。
