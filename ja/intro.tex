...
なぜデータ構造はこんなにも重要なのだろう？考えてみるとデータ構造と普段から接していることに気づく。
\begin{itemize}
...
次節では多くのデータ構造がサポートする操作を見ていく。ちょっとしたプログラミング経験があれば、これらを正しく実装するのは難しくない。データを配列または線形リストに格納し、配列または線形リストの全要素を見てまわり、要素を追加したり削除したりすればよいのだ。
...
解決策はもちろん、データ構造内のデータを注意深く格納して、各操作がすべてのデータを見て回る必要がないようにすることだ。最初はムリに思えるが、データ構造に含まれるデータ数にかかわらず、平均して2つのデータだけを参照すればすむデータ構造をのちに紹介する。1秒あたり10億の命令を実行できるとして、10億個のデータ（兆、京、垓であっても）を含むデータ構造を検索するのにわずか$0.000000002$秒しかかからないのだ。
