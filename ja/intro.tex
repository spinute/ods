...
Big-Oh表記法は、より複雑な関数を分析することを可能にする、より高いレベルで推論することを可能にします。 2つのアルゴリズムの実行時間が同じであれば、どちらが速いか分かりませんし、明確な勝者がないかもしれません。あるマシンではより速く、別のマシンでは速くなるかもしれません。しかし、2つのアルゴリズムが大きく異なる実行時間を持っている場合、実行時間の短い方が$ \ mathtt {n} $の十分な大きさの方が高速であることが確信できます。
...
この定義は、私たちが本当に気にしている状況を捕捉します。引数$ n_1、\ ldots、n_k $ make $ g $が大きな値をとるときです。 この定義は、$ f（n）$が$ n $の増加関数であるときの$ O（f（n））$の単変量定義にも同意します。 読者は、これが我々の目的のために働くが、他のテキストは多変量関数および漸近表記を異なる方法で扱うことがあることを警告すべきである。
...
$ \ displaystyle \ mathrm {E} [X] = \ sum_ {U \} x \ cdot \ Pr \ {X = x \} \ enspace。 $

ここで$ \ Pr \ {\ mathcal {E} \} $はイベント$ \ mathcal {E} $が発生する確率を示します。この本のすべての例では、これらの確率はランダム化されたデータ構造によって行われたランダムな選択に関してのみです。構造体に格納されたデータやデータ構造体上で実行された操作のシーケンスがランダムであるという前提はありません。

期待値の最も重要な特性の1つは、期待の直線性です。任意の2つの確率変数$ X $と$ Y $に対して、

$ \ displaystyle \ mathrm {E} [X + Y] = \ mathrm {E} [X] + \ mathrm {E} [Y] \ enspaceです。 $

より一般的には、任意の確率変数$ X_1、\ ldots、X_k $、

\\ {0} $ \ displaystyle \ mathrm {E} [X_i] \ enspace。 $

期待の直線性は、複雑な確率変数（上記の式の左辺のようなもの）をより単純な確率変数の和（右辺）に分解することを可能にする。

繰り返し使用する便利なトリックは、指標のランダム変数を定義することです。これらのバイナリ変数は、何かを数えたい場合に便利で、例で最もよく示されています。私たちが公正なコイン$ k $回を投げたと仮定し、コインが頭になる予想数を知りたいとします。直感的には、答えは$ k / 2 $ですが、期待値の定義を使って証明しようとすると、

$ \ displaystyle \ mathrm {E} $ $ \ displaystyle = \ sum_ {i = 0} ^ k i \ cdot \ Pr \ {X = i \} $
  $ \ displaystyle = \ sum_ {i = 0} ^ k i \ cdot \ binom {k} {i} / 2 ^ k $
    ^ {k-1} \ bin {k-1} {i} / 2 ^ k $
	  $ \ displaystyle = k / 2 \ enspace。$

	  これは、$ \ Pr \ {X = i \} = \ binom {k} {i} / 2 ^ k $を計算するのに十分なことが分かっていなければならず、2項アイデンティティ$ i \ binom {k} {i } = {k-1} {i} $と$ \ sum_ {i = 0} ^ {k} \ binom {k} {i} = 2 ^ {k} $である。

	  インジケータ変数と期待値の直線性を使用すると、作業がはるかに簡単になります。 \ {1、\ ldots、k \} $の各$ iに対して、インジケータの確率変数を定義する

	  $ \ ifstin {\ begin {cases} $ 1 $コイントスが頭の場合は\\ 0＆\ text {そうでなければ} \ end {cases} $

	  その後、

	  $ \ displaystyle \ mathrm {E} [I_i] =（1/2）1 +（1/2）0 = 1/2 \ enspace。 $

	  今、$ X = \ sum_ {i = 1} ^ k I_i $だから

	  $ \ displaystyle \ mathrm {E} \ left {\ sum_ {i = 1} ^ k I_i \ right] $ \ displaystyle \ mathrm {
	    $ \ displaystyle = \ sum_ {i = 1} ^ k \ mathrm {E} [I_i] $
		  $ \ displaystyle = \ sum_ {i = 1} ^ k 1/2 $
		    $ \ displaystyle = k / 2 \ enspace。$

			これはもう少し長持ちですが、魔法のアイデンティティーを知っていることや、自明ではない確率を計算する必要はありません。さらに良いことに、各コインが$ 1/2 $の確率で頭として登場するため、コインの半分が正確に頭に浮かんでくることがわかるという直感に同意します。
