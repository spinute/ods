...
次節では多くのデータ構造がサポートする操作を見ていく。ちょっとしたプログラミング経験があれば、これらを正しく実装することは難しくない。データを配列または線形リストに格納し、配列または線形リストの全要素を見てまわり、要素を追加したり削除したりすればよいのだ。 % Linked list == 線形リスト？ not sure.
...
\item 計算モデル % 計算のモデル or 計算モデル
\item 正しさと実行時間、メモリ使用量
\item 残りの章の概要
\item サンプルコードと記号の表記法　% 組版の規則 
\end{itemize}
これらの背景知識があってもなくても、いったん気軽に飛ばし必要に応じて戻り読みしてもよい。

%%% TODO: インターフェイス vs. インターフェース: どっちでも良いがどちらかに統一する必要がある。
\section{インターフェイス} 
\seclabel{インターフェイス}
...
#remove()#操作の引数はないことに注意する。#Queue#は\emph{取り出し規則}に従って削除する次の要素を決める。取り出し規則は色々と考えられるが、主なものとしてはFIFOや優先度、LIFOなどがある。 % queueing discipline = 取り出し規則
...
#Queue#から最小の要素を削除する。同点要素が複数あるときは、そのうちのいずれかを任意に選ぶ。これは病院の救急室で重症患者を優先的に治療することに似ている。患者が到着したら、症状を見積もってから待合室で待機していてもらう。医師の手が空くと、最も重篤な患者から治療する。優先度付き#Queue#における#remove()#操作を、他の教科書ではよく#deleteMin()#などと呼んでいる。 % YJ: I have never seen deleteMin though.
...
この本では、FIFO #Queue#や#Stack#、#Deque#のインターフェースについての話はほとんどしない。なぜなら、これらのインターフェシウは#List#インターフェースにまとめられるためだ。図1.4に示す#List#\index{List@#List#}は、値の列$#x#_0,\ldots,#x#_{#n#-1}$を表現する。
...
今述べた定義で、探したい#x#と、見つかるかもしれない集合の要素#y#の区別を小難しく感じるかもしれない。これを区別する理由は、#x#と#y#は実は異なるオブジェクトだが等しいと判定されるかもしれないためだ。異なるオブジェクトを等しいと判定するインターフェイスは、キーを値に対応づける\emph{辞書}(\emph{マップ})を作るのに便利なのだ。 % YJ need better translation
\index{dictionary}%
\index{map}%
...
\]
二項係数$\binom{n}{k}$は、大きさ$n$の集合における大きさ$k$の部分集合の個数である。すなわち、集合$\{1,\ldots,n\}$から相異なる$k$個の整数を取り出すときの場合の数である。 % YJ 元の文でもi.e.とあるが、これは定義ではなく一つの例なのでは？すなわち、と言ってしまってよいのだろうか。「例えば」の方が正確では。
...
\]
グラフ上で考えると、$n$が十分に大きくなると$c\cdot f(n)$が$g(n)$よりも大きくなるか同じになる関数$g(n)$を集めたものがこの集合だ。 % 「上から抑えられる」は既に知識がある人には伝わるが、初学者には伝われないのでは。 % TODO: need revision
...
Java Collections Frameworkの#List#・#Set#・#Map#・#SortedSet#・#SortedMap#をシンプルにしたのである。

この章で扱った漸近記法・対数・階乗・スターリングの近似・確率論の基礎などは、Leyman, Leighton, and Meyer\cite{llm11}の素晴らしい（そしてフリーの）本が扱っている。
丁寧な微積分の教科書として、無料で手に入るThompson\cite{t14}の古典的な教科書がある。
この本では指数や対数の形式的な定義が書かれている。

基礎的な確率論についての、特にコンピュータ・サイエンスに関連するものとしてRoss\cite{r01}の教科書はおすすめである。
...
以下の問題はテキストの入力を一行ずつ読み、各行で適切なデータ構造の操作を実行することで解いてほしい。ただしファイルが百万行であっても数秒以内に処理できる程度に効率的な実装でなければならないものとする。
  ...
  \begin{enumerate}
    ...
    \item 入力をすべて読み、偶数番目の行の後に奇数番目の行を出力せよ。（なお、最初の行を0行目と数える。）
...
\begin{exc}
  \index{Dyck word}%
  \emph{Dyck word}とは+1, -1からなる列で、先頭を含む部分列（プレフィックス）の和がいずれも非負であるものである。% XXX: 曖昧でなく書けるか?
