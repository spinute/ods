...
\notpcode{
この本のコードサンプルは\langで書いた。
しかし、\langに親しみのない人も読めるようシンプルに書いたつもりだ。
...
完全な実装に興味のある読者はこの本に付属の\langソースコードを見てほしい。

この本は数学的な実行時間の解析と、対象のアルゴリズムを実装した\langのコードとを共に含む。そのためソースコードと数式で同じ変数が出てくる。
このような変数は同じ書式で書く。
