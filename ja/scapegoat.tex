...
部分的な再構築とは部分木を一度分解し、完全二分木\footnote{訳注：10章で現れるが、完全二分木とは任意の葉の高さの差が高々1である木のことである。}として再構築するプロセスである。
...
%% spinute: perfectly balanced は、size balanced ではなく、complete のことではなかろうか（以下で紹介している構成方法だと、size-balanced かつ complete な木になる）
ノード#u#を根とする部分木を完全二分木に再構築するやり方はたくさんある。
もっとも単純なやり方のひとつは#u#の部分木を辿りすべてのノードを配列#a#に集め、#a#から再帰的に完全二分木を構築するやり方だ。
% m = floor(a.length/2)?
...
今はこれを事実として認めることにする。
scapegoatである#w#が見つかれば、#w#を根とする部分木を完全二分木に再構築すればよい。
\myeqref{scapegoat}より、#u#を加える前には#w#の部分木は完全二分木ではない。
