...
この節では\emph{償却解析}として知られる方法でこれを解決する。 % TODO: YJ check the translation of other textbooks if 償却 is the word used widely
% caprice to YJ Wikipediaに「償却解析 (Amortized analysis)」って項目あるからいいのでは？
...
\begin{proof}
#resize()#が呼ばれるとき、その前の#resize()#呼び出しから#add#・#remove#が実行された回数は$#n#/2-1$回以下である。
...
ひとつ前の（つまり、i-1回目の）#resize()#の直後では#a#の要素数は$#a.length/2#-1$以下であった。
% XXX この a.length/2-1 以下ってどうして言えるのか？
% 本書中の resize() は、縮小する場合のサイズについて言及していない
% そのため、
% a.length = 16 から a.length = 32     に i-1 回目の resize
% a.length = 32 から a.length = 32 /3  に i   回目の resize
% をした際に a.length がいくつになるのか不明でこの議論は成り立ってないと思う
%
\footnote{この数式における${}-1$は、特別なケース$#n#=0$かつ$#a.length# = 1$を考慮したものだ。}
% いや、 n = 0 かつ a.length = 1 で a.length/2 - 1 を計算したら0未満になるでしょ、何言ってんの
...
\begin{thm}\thmlabel{arraystack}
  ...
  \begin{itemize}
    ...
    \item #add(i,x)#・#remove(i)#の実行時間は$O(#n#-#i#)$である。
  \end{itemize}
...
\begin{figure}
  ...
  \end{center}
  \caption{#ArrayQueue#に対する#add(x)#・#remove(i)#の実行例。矢印は要素のコピーを表す。#resize()#の発生する呼び出しにはアスタリスクを付した。}
  \figlabel{arrayqueue}
\end{figure}
