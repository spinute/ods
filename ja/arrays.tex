...
この節では、平方根の算出が効率的に実装できることを示す。特に「長さが$O(\sqrt{#n#})$の２つの配列#sqrttab#と#logtab#を作り、実行時間が$O(\sqrt{#n#})$である前処理」のあとで、どんな整数$#x#\in\{0,\ldots,#n#\}$についても定数時間で$\lfloor\sqrt{#x#}\rfloor$を計算できることを示す。
