...
ここで素体上の多項式とは、#p#を素数として、$#x#_i\in\{0,\ldots,#p#-1\}$が係数であるような普通の多項式のことを指す。% 下記の話を読むといわゆる「素体」を指している -kshikano
...
実装の出来不出来によっては、コードを見るだけで実行時間がわかる場合もあれば、挿入や検索をしてみて実行時間を測るコードを書く必要があるかもしれない（これはWebサーバへのDoS攻撃に使われることがある\cite{cw03}）。
